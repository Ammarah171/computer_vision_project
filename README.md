...
Detection alone answers: *"what objects are in this frame?"*  
Tracking answers: *"what objects are in this frame AND are they the same objects from the previous frame?"*

Without tracking:
```
Frame 1: person detected at (100, 200)
Frame 2: person detected at (105, 210)
→ Are these the same person? Detection cannot tell.
```

With tracking:
```
Frame 1: ID:1 person at (100, 200)
Frame 2: ID:1 person at (105, 210)  ← same person, moved 5 pixels
```

Tracking enables counting **unique individuals across an entire video** rather than just per-frame totals.

#### DeepSORT Pipeline (4 steps per frame)

**Step 1 — YOLOv8 Detection**  
YOLOv8 runs on each frame producing raw bounding boxes, confidences, and class IDs for classes [0, 1, 3].

**Step 2 — Kalman Filter (prediction)**  
Predicts where each currently tracked object will be in the next frame based on its position and velocity. When an object disappears (occluded) for up to `max_age=30` frames the filter keeps predicting its position — preventing track loss during brief occlusions.

**Step 3 — Hungarian Algorithm (matching)**  
Finds the optimal one-to-one matching between Kalman-predicted positions and new detections:
```
Predicted:  Track1:(105,210)  Track2:(340,180)  Track3:(520,400)
Detected:   Det_A:(104,212)   Det_B:(519,398)   Det_C:(338,182)

Matched:
  Track1 ↔ Det_A  (3px apart)
  Track2 ↔ Det_C  (4px apart)
  Track3 ↔ Det_B  (3px apart)
```
Unmatched detections become new tentative tracks. Unmatched existing tracks are kept alive for `max_age` frames before being dropped.

**Step 4 — Re-ID Embeddings (appearance verification)**  
MobileNet extracts a 128-dimensional appearance fingerprint from each detection. Matches are verified using cosine distance (`max_cosine_distance=0.3`) — if two appearances differ by more than 0.3 they will not be matched even if spatially close. This prevents ID switches when two people walk close together.

New tracks must appear in `n_init=3` consecutive frames before receiving a confirmed permanent ID — preventing false positives from triggering track IDs.

#### Visual Output

- Each confirmed track is drawn with a unique consistent color
- Labels show `ID:{n} class` for each tracked object
- **Trajectory trails** — the last 40 center positions are connected with lines that fade from dark (old) to bright (new), showing movement direction
- Bottom of frame shows cumulative unique object count across the entire video

#### Detection vs Tracking Comparison

| Property | `detect.py` | `track.py` |
|---|---|---|
| Works on | Images or video | Video only |
| Count meaning | Objects visible right now | Unique objects ever seen |
| Object identity | None — just boxes | Persistent ID per object |
| Handles occlusion | No | Yes — Kalman keeps track alive |
| Output | Annotated images | Annotated video with trajectories |
...
print(f"Unique car tracks   : {sum(1 for c in track_classes.values() if c != 0)}")
```
